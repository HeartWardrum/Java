...
视图在创建的时候，可以在使用 as 字段别名 来重新定义视图中的字段名
...
注意：rownum的条件过滤是针对原始数据，如果需要排序，那么得先排序，在排序的结果集上加rownum
...
rownum的条件判断只支持`<`和`<=`，我们可以在当前结果集查询的时候给它起别名，相当于造了一个普通字段，支持各种比较运算符，然后再外层对别名进行条件判断。
...
将一组逻辑相关的变量整合到一块，做成一个类型，再由该类型定义一个变量，那么该变量就可以接收一组值，使用的时候，使用变量名，属性即可
定义格式：
...
declare
 type my_record is record(
  ...
  begin
    ...
    into v_emp
    from emp e where e.empno = '7369';
    dbms_output.put_line(v_emp.v_eno||','||v_emp.v_ename||','||v_emp.v_sal);
end;
~~~
...
declare
  --定义游标
  cursor my_cur is
    select me.ename, me.sal from my_emp me;
begin
  for c in my_cur loop
  
    if c.sal < 1000 then c.sal:=c.sal*1.2;
    elsif c.sal<=1900 then c.sal:=c.sal *1.15;
    else c.sal:=c.sal * 1.1;
    end if;
      dbms_output.put_line(c.ename || ',' || c.sal);
      update my_emp set sal = c.sal where ename = c.ename;
